...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  //auto look_ahead_distance = velocity_mag*_lookahead_time + 0.5*accel_mag*std::pow(_lookahead_time, 2);  // <- Fix This
  auto look_ahead_distance = std::abs(std::pow(velocity_mag, 2) / (2 * _max_accel));  // <- Fix This
